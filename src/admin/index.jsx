...
const NAV_ITEMS = [
  ...
  { id: 'trainings', name: 'Trainings', icon: 'training', description: 'Manage training sessions' },
  { id: 'trainers', name: 'Trainers', icon: 'trainers', description: 'View trainers directory' },
  { id: 'participants', name: 'Participants', icon: 'participants', description: 'Manage participants' },
  { id: 'logout', name: 'Logout', icon: 'quit', description: 'Sign out of system' },
];
...
const NavIcon = ({ icon, isActive }) => {
  ...
  switch (icon) {
	case 'home':
	  return (
		<svg className={baseStyle} viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export default ({currentUser})=> {
  ...
  const [activeSection, setActiveSection] = useState(NAV_ITEMS[3].id);
  ...
  const renderContent = () => {
	switch (activeSection) {
	  ...
	  case NAV_ITEMS[3].id:
		return <Participants />;
	  default:
		return <div className="flex-1 flex items-center justify-center">Select a section</div>;
	}
  };
	...
	<div className="flex font-sans min-h-screen bg-gray-50">
	  ...
	  >
		...
		<div className="px-6 py-5 flex items-center justify-center">
		  <svg className="w-8 h-8 mr-2" viewBox="0 0 24 24" fill="none" stroke={THEME.white} strokeWidth="2">
		...
		<div className="flex items-center p-6 mt-2 border-b border-white border-opacity-10">
		  <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center mr-4 shadow-inner overflow-hidden">
			<svg className="w-full h-full" viewBox="0 0 24 24" fill="none" stroke={THEME.dark} strokeWidth="1.5" aria-hidden="true">
			  <path strokeLinecap="round" strokeLinejoin="round" d="M17.982 18.725A7.488 7.488 0 0012 15.75a7.488 7.488 0 00-5.982 2.975m11.963 0a9 9 0 10-11.963 0m11.963 0A8.966 8.966 0 0112 21a8.966 8.966 0 01-5.982-2.275M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" />
			</svg>
		  </div>
		  <div>
			<h3 className="text-lg font-semibold text-white text-opacity-90 tracking-wide">{currentUser.login.charAt(0).toUpperCase() + currentUser.login.slice(1)}</h3>
		  ...
		  <div className="flex justify-between items-center">
			<div>
			  <h2 className="text-2xl font-medium text-gray-800">
				{NAV_ITEMS.find(item => item.id === activeSection)?.name || 'Dashboard'}
			  </h2>
			  <nav className="text-sm" aria-label="Breadcrumb">
				<ol className="list-none p-0 inline-flex">
				  ...
				  <li>
					<span className="text-gray-800">{NAV_ITEMS.find((item,index) => item.id === activeSection && index!=0)?.name || 'Dashboard'}</span>
				  </li>
				</ol>
			  </nav>
			</div>
