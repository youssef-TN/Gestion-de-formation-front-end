...
import { fetchTrainings, addTraining, updateTraining, deleteTraining } from '../api/trainingApi';
...
export default () => {
  ...
  const handleDelete = async (id) => {
    const confirm = window.confirm("Are you sure you want to delete this training?");
    if (confirm) {
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingTraining) {
        ...
        setTrainings(prev => prev.map(t => t.id === editingTraining.id ? updated : t));
      } else {
  ...
  const getFilteredAndSortedTrainings = () => {
    ...
    const filtered = trainings.filter(training => {
      const matchesYear = !filters.year || training.year.toString() === filters.year;
      const matchesField = !filters.field || training.field === filters.field;
      const matchesDuration = !filters.duration || training.duration.toString()=== filters.duration;
      
      // Budget range filtering
      const budget = parseFloat(training.budget);
      const matchesMinBudget = !filters.minBudget || budget >= parseFloat(filters.minBudget);
      const matchesMaxBudget = !filters.maxBudget || budget <= parseFloat(filters.maxBudget);
      
      // Search query
      const matchesSearch = !searchQuery || 
        training.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
        training.field.toLowerCase().includes(searchQuery.toLowerCase()) ||
        training.duration.toLowerCase().includes(searchQuery.toLowerCase());
      
      return matchesYear && matchesField && matchesDuration && 
             matchesMinBudget && matchesMaxBudget && matchesSearch;
    });
    
    // Then sort the filtered trainings
    return [...filtered].sort((a, b) => {
      if (sortConfig.key === 'budget') {
        // Sort numerically for budget
      ...
      } else if (sortConfig.key === 'year' || sortConfig.key === 'duration') {
  ...
  return (
    <div className="p-6 space-y-6 bg-[#F6EEE0]/10 ">      
      <h2 className="text-3xl font-semibold text-gray-800 drop-shadow-md">Training Management</h2>
          ...
          <tbody className="bg-white text-sm text-gray-700">
            {loading ? (
              ...
              filteredAndSortedTrainings.map((training) => (
                <tr key={training.id} className="border-b border-gray-200 hover:bg-[#F6EEE0]/20">
      ...
      <div className="flex justify-between items-center text-sm text-gray-600">
        <span>Showing {filteredAndSortedTrainings.length} of {trainings.length} trainings</span>
      </div>
